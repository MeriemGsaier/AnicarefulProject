...
@Component({
  selector: 'app-ajout-contenu-info',
  templateUrl: './ajout-contenu-info.component.html',
  styleUrls: ['./ajout-contenu-info.component.scss']
})
export class AjoutContenuInfoComponent implements OnInit {
...
ajouterContent(){
  ...
  if (!this.contentForm.value.titre || !this.contentForm.get('photo').value || !this.contentForm.value.description ) {
  ...
  this.contenuInformatifService.add(formData).subscribe((response : any)=>{
    this.responseMessage=response?.message;
    Swal.fire('Succès', this.responseMessage, 'success');
    this.router.navigate(['/prop-dashboard/contenu-informatif']);
